...
for(i in 1:4)
{
  ...
  pdf(paste("Lyx/fig/k_particle_",datnames[i],"_",npar,".pdf",sep=""),width=7,height=3.5*2)
...
pdf(paste("Lyx/fig/k_particle_sim_",npar,".pdf",sep=""),width=7,height=3.5*2)
...
p_plot=ggplot(dat, aes(x)) + 
  ...
  scale_color_brewer(palette="Set1")+
  scale_fill_manual(values=c("black"),breaks=c("95 % CI, k"))+
  theme_bw()+theme(legend.position="none")+ylab("")+xlab("t")
