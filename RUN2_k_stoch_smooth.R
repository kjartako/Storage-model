...
for(i in 1:ndataset)
{
  ...
  theta=readRDS(file=paste("mcmc/monthly/",datnames[i],"_10000_",C,".rds",sep=""))$theta[(nburnin+1):nMCMC,]
  ...
  set.seed(1234)
  tmp=SMC_sample_smooth(postmean_transformed,p,npar=npar,nrep=nrep,N,xleft,xright,grid,vals,a,C,r,v_x,int.grid,int.wts,nint,ngrid,30,0.5)
  rep=tmp$rep
  x=tmp$Xsmooth
  
  k=matrix(NA,nrow=nrep,ncol=N)
  pkdiff=matrix(NA,nrow=nrep,ncol=N)
  for(j in 1:N)
  {
    for (m in 1:nrep)
    {
  ...
  res=c(res,list(list(name=datnames[i],p=p,rep=rep,x=x,k=k,pkdiff=pkdiff)))
}

saveRDS(res, file=paste("mcmc/monthly/k_stoch_smooth05_",C,".rds",sep=""))
